...
export const getMovieCastUseCase = async ( fetcher: HttpAdapter, movie_id: number ): Promise<Cast[]> => {
  try {

    const { cast } = await fetcher.get<MovieDBCastResponse>(`/${movie_id}/credits`);
